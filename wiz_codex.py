...
def run_dir_scan():
    ...
    def find_menu_struct_candidates(pm, regions):
        """
        有効なmenu_state構造体の候補を全メモリから抽出し、構造体先頭アドレスのリストとして返す。
        """
        region_data = read_regions_bytes(pm, regions)
        raw_matches = scan_menu_struct_offsets(region_data)
        candidates = [
            addr for addr, data, i in raw_matches
            if is_valid_menu_struct(data, i)
        ]

        print(f"✅ 有効な menu_struct 候補数: {len(candidates)} 件")
        return candidates
    ...
    def is_valid_menu_struct(data, index):
        """
        指定インデックスの構造体が整合性のある menu_struct かどうかを判定する。

        Parameters:
            data: bytes列（readされたメモリ領域）
            index: data内の構造体先頭インデックス

        Returns:
            bool: True（整合性あり） or False（異常値あり）
        """
        try:
            dir_val = data[index + OFFSET_DIR]
            x = data[index + OFFSET_X]
            y = data[index + OFFSET_Y]
            floor = data[index + OFFSET_FLOOR]
            return (0 <= dir_val <= 3) and (0 <= x <= 23) and (0 <= y <= 23) and (0 <= floor <= 99)
        except IndexError:
            return False
...
class MapApp:
    ...
    def _run_scan_thread(self):
        ...
        # --- 説明 ---
        # menu_state を定期的に監視し、"map_open" 状態（MENU_STATE_TABLE参照）になったときのみに
        # capture_map_screenshot() を1回だけ実行する。
        # GUI側の自動キャプチャ設定が ON のときのみ動作し、連続実行を防ぐため前回状態と比較する。
